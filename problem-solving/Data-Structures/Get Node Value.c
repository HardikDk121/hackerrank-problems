...
int getNode(SinglyLinkedListNode* llist, int positionFromTail){
    struct SinglyLinkedListNode *current,*tail;
    for (current =llist;current !=NULL; current = current -> next)
    {
        tail = current;
        for (int position = -1; position!=positionFromTail && tail!=NULL; position++)
        {
            tail = tail ->next; 
        } 
        if (tail == NULL)
