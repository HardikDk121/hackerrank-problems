...
bool compare_lists(SinglyLinkedListNode* head1, SinglyLinkedListNode* head2)
{
    ...
    if (head1 == NULL || head2==NULL) 
        return 0;
    SinglyLinkedListNode *link1node ,*link2node;
    
    for (
    link1node = head1 , link2node = head2;
    link1node!=NULL && link2node!=NULL;
    link1node = link1node ->next , 
     link2node = link2node -> next
     )
    {   
        if(link1node -> data != link2node -> data ) 
            return 0;      
    }
    return (link1node  == NULL  && link2node == NULL);
}
